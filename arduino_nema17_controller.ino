...
const int MAX_STEPS = STEPS_PER_REVOLUTION * 2;  // Máximo 2 vueltas (720 grados)
...
void moveToPosition(int degrees, int dwellSeconds) {
...
void moveStepper() {
  ...
  if (currentTime - lastStepTime >= STEP_DELAY) {
    if (currentPosition != targetPosition) {
      ...
      // Mantener posición por el tiempo especificado
      if (dwellSeconds > 0) {
...
// Función auxiliar para mover pasos individuales
...
// Función para calibración manual
void calibrateMotor() {
  Serial.println("=== CALIBRACIÓN DEL MOTOR ===");
  Serial.println("Moviendo 360 grados en sentido horario...");
  
  digitalWrite(DIR_PIN, HIGH);
  stepMotor(STEPS_PER_REVOLUTION);
  currentPosition += STEPS_PER_REVOLUTION;
  
  delay(2000);
  
  Serial.println("Moviendo 360 grados en sentido antihorario...");
  digitalWrite(DIR_PIN, LOW);
  stepMotor(STEPS_PER_REVOLUTION);
  currentPosition -= STEPS_PER_REVOLUTION;
