...
"""
main_window.py
- Contiene la clase App (GUI PySide6) y el arranque principal de la interfaz.
"""
import time
import threading
...
from src.domain.services import (
    SimpleThresholdMatcher,
    FingerprintAuthService,
    FingerprintTemplate as DomainTemplate,
)
from src.infra.hardware.matcher import PyFingerprintHardwareMatcher
from src.app.services.fingerprint_auth import FingerprintAuthAppService
...
class App(QWidget):
    def __init__(self):
        ...
        self.people: List = []  # Personas cargadas desde SICEFA (para validación de rol)
        ...
        # Matcher de dominio (fallback para pruebas/simulador)
        self.matcher = SimpleThresholdMatcher()
        self.auth_service = FingerprintAuthService(self.matcher)
        # Servicio de aplicación (por defecto usa matcher simple; con sensor se reemplaza a hardware)
        self.app_auth_service = FingerprintAuthAppService(self.matcher)
    ...
    def _build_ui(self):
        ...
        # Estado oculto para mantener referencias
        ...
        # Botón de programación en la esquina inferior derecha
    ...
    def _create_schedule_button(self):
        ...
        if self.schedule_button is None:
            ...
            self.schedule_button.setFixedSize(70, 70)
    ...
    def _position_schedule_button(self):
        ...
        if self.schedule_button:
            self.schedule_button.move(self.width() - 80, self.height() - 80)
    ...
    def _auto_load_candidates(self):
        """
        Carga candidatos desde SICEFA usando repositorios desacoplados.
        Fallback: intenta el método previo y, si falla, usa datos de ejemplo.
        """
        try:
            print("🔄 Conectando a la base de datos SICEFA (repositorios)...")
            if self.app_auth_service:
                people, templates = self.app_auth_service.load_people_and_templates()
            else:
            ...
            if templates:
                ...
                for tpl in templates:
                    name = names.get(tpl.person_id, f"User {tpl.person_id}")
                    candidates.append((tpl.person_id, name, tpl.template))

            if not candidates:
                print("⚠️ Repositorios devolvieron 0 candidatos. Probando método previo...")
                try:
            ...
            if self.candidates:
                ...
                self._set_status(
                    f"🟢 {len(self.candidates)} CANDIDATOS CARGADOS DESDE SICEFA - SISTEMA LISTO"
                )
    ...
    def _real_fingerprint_worker(self):
        try:
            ...
            try:
                ...
                # Convertir candidatos actuales a DomainTemplate
                candidate_templates = [
                    DomainTemplate(person_id=pid, template=tpl)
                    for (pid, name, tpl) in self.candidates
                    if tpl
                ]
                people = getattr(self, "people", [])

                # 1) Intentar matcher de hardware (usa buffer1 ya cargado)
                result = None
                try:
                    hw_service = FingerprintAuthAppService(PyFingerprintHardwareMatcher(sensor))
                    result = hw_service.authenticate(
                        captured_template=DomainTemplate(person_id=-1, template=[]),
                        requested_role=self.requested_interface,
                        threshold=threshold,
                        topn=topn,
                        people=people,
                    ...
                    print(f"⚠️ Matcher de hardware falló, usando fallback: {hw_err}")

                # 2) Fallback a matcher simple descargando características
                if result is None or not result.ok:
                    cap_tpl = sensor.downloadCharacteristics()
                    captured = DomainTemplate(person_id=-1, template=cap_tpl)
                ...
                if result.ok and result.person:
                    ...
                    print(f"❌ Sin coincidencias válidas o rol denegado. Motivo: {result.reason if result else 'unknown'}")
