...
from src.utils.db_utils import get_weekly_schedule
...
class ScheduleCard(QFrame):
    ...
    def setup_card(self):
        ...
        self.setStyleSheet("""
            QFrame {
                background-color: #f8f9fa;  /* Cambiado de #2d2d2d a un gris muy claro */
                border: 1px solid #dee2e6;  /* Cambiado de #404040 a un borde más claro */
                border-radius: 8px;
                margin: 2px;
            }
            QFrame:hover {
                border: 1px solid #0d6efd;  /* Cambiado de #4a9eff a un azul más estándar */
                background-color: #ffffff;  /* Cambiado de #353535 a blanco */
            }
        """)
        ...
        day_label.setStyleSheet("""
            QLabel {
                color: #0d6efd;  /* Cambiado de #4a9eff a un azul más estándar */
                font-size: 14px;
                font-weight: 600;
            }
        """)
        ...
        time_label.setStyleSheet("""
            QLabel {
                color: #6c757d;  /* Cambiado de #a0a0a0 a un gris medio */
                font-size: 12px;
            }
        """)
        ...
        instructor_label.setStyleSheet("""
            QLabel {
                color: #212529;  /* Cambiado de #ffffff a texto oscuro */
                font-size: 14px;
                font-weight: 500;
            }
        """)
        ...
        environment_label.setStyleSheet("""
            QLabel {
                color: #198754;  /* Cambiado de #10b981 a un verde más estándar */
                font-size: 12px;
                font-weight: 500;
            }
        """)
...
class ScheduleInterface(QWidget):
    ...
    def show_schedule_interface(self):
        ...
        if self.schedule_win is None:
            # Crear ventana en pantalla completa real
            self.schedule_win = CleanCloseDialog(self.parent)
            self.schedule_win.setWindowTitle("Sistema de Programación - CEFA")
            self.schedule_win.setModal(False)
            self.schedule_win.setStyleSheet("""
                QWidget {
                    background-color: #ffffff;  /* Cambiado de #1e1e1e a blanco */
                    color: #333333;  /* Cambiado de #e0e0e0 a texto oscuro */
                    font-family: 'Segoe UI', Arial, sans-serif;
                }
            """)
            
            # PANTALLA COMPLETA REAL (sin barra de tareas de Windows)
            self.schedule_win.showFullScreen()
        ...
        # Ya está en pantalla completa real
        print("✅ Interfaz de programación mostrada en pantalla completa real")
    ...
    def _create_clean_control_panel(self, parent_layout):
        ...
        search_label.setStyleSheet("""
            QLabel {
                color: #212529;  /* Cambiado de #e0e0e0 a texto oscuro */
                font-size: 14px;
                font-weight: 500;
            }
        """)
        ...
        self.search_input.setStyleSheet("""
            QLineEdit {
                background-color: #ffffff;  /* Cambiado de #1e1e1e a blanco */
                border: 1px solid #ced4da;  /* Cambiado de #404040 a un borde más claro */
                border-radius: 6px;
                padding: 8px 12px;
                color: #212529;  /* Cambiado de #e0e0e0 a texto oscuro */
                font-size: 14px;
            ...
            QLineEdit:focus {
                border: 1px solid #0d6efd;  /* Cambiado de #4a9eff a un azul más estándar */
                background-color: #ffffff;  /* Cambiado de #252525 a blanco */
            }
        """)
        ...
        refresh_btn.setStyleSheet(self._get_clean_button_style("#0d6efd"))  # Cambiado de #4a9eff a un azul más estándar
        refresh_btn.clicked.connect(self._refresh_schedule)
        
        clear_btn = QPushButton("Limpiar")
        clear_btn.setStyleSheet(self._get_clean_button_style("#dc3545"))  # Cambiado de #ef4444 a un rojo más estándar
    ...
    def _create_clean_content_area(self, parent_layout):
        ...
        scroll_area.setStyleSheet("""
            ...
            QScrollBar::handle:vertical {
                background-color: #0d6efd;  /* Cambiado de #4a9eff a un azul más estándar */
                border-radius: 6px;
                min-height: 20px;
            }
            QScrollBar::handle:vertical:hover {
                background-color: #0b5ed7;  /* Un tono ligeramente más oscuro para el hover */
            }
        """)
        ...
        self.cards_container.setStyleSheet("background-color: #ffffff;")  # Asegurar fondo blanco
    ...
    def _display_schedule_cards(self):
        ...
        if not self.schedule_data or len(self.schedule_data) == 0:
            ...
            no_data_frame.setStyleSheet("""
                QFrame {
                    background-color: #f8f9fa;  /* Cambiado a un gris muy claro para coincidir con el diseño */
            ...
            no_data_label.setStyleSheet("""
                QLabel {
                    color: #6c757d;  /* Cambiado de #00d4ff a un gris medio para mejor contraste en fondo blanco */
                    font-size: 16px;
                    font-weight: bold;
